...
#
# yfinance's native intraday intervals don't include "3m" or "4h" — for
# those two, we fetch the nearest finer native interval and resample with
# pandas. This is a REAL aggregation of REAL data (not an approximation
# of the number), but candle boundaries may not perfectly line up with
# what a broker's own native 4h candles would show (broker session
# boundaries vary) — worth knowing if you're cross-checking against MT5.
# ---------------------------------------------------------------------------
_YF_NATIVE_INTERVALS = {"1m", "5m", "15m", "30m", "1h", "1d"}

# For intervals yfinance doesn't support natively: (base interval to fetch, pandas resample rule)
_YF_RESAMPLE_FROM = {
    "3m": ("1m", "3min"),
    "4h": ("1h", "4h"),
}

# yfinance's own spelling for each of OUR interval names
_YF_INTERVAL_SPELLING = {
    "1m": "1m", "5m": "5m", "15m": "15m", "30m": "30m", "1h": "60m", "1d": "1d",
}

# yfinance limits how far back intraday intervals can go — request a
# lookback period comfortably larger than what ROLLING_WINDOW_SIZE candles
# at each interval would span.
_YF_LOOKBACK_PERIOD = {
    "1m": "7d", "5m": "60d", "15m": "60d", "30m": "60d", "1h": "730d", "1d": "730d",
}


def _resample_ohlcv(df: pd.DataFrame, rule: str) -> pd.DataFrame:
    """Aggregates finer candles into coarser ones (e.g. 1m -> 3min, 1h -> 4h)."""
    indexed = df.set_index("open_time")
    agg = indexed.resample(rule, label="left", closed="left").agg({
        "open": "first", "high": "max", "low": "min", "close": "last", "volume": "sum",
    })
    agg = agg.dropna(subset=["open"])  # drop empty bins (e.g. weekend gaps)
...
def get_gold_klines_yfinance(interval: str = None, limit: int = None) -> pd.DataFrame:
    ...
    if interval in _YF_NATIVE_INTERVALS:
        fetch_interval = interval
        needs_resample = False
    elif interval in _YF_RESAMPLE_FROM:
        fetch_interval, resample_rule = _YF_RESAMPLE_FROM[interval]
        needs_resample = True
    else:
        raise ValueError(f"Unsupported interval for gold: {interval!r}")

    yf_interval = _YF_INTERVAL_SPELLING[fetch_interval]
    period = _YF_LOOKBACK_PERIOD[fetch_interval]

    hist = yf.Ticker(config.GOLD_YF_TICKER).history(period=period, interval=yf_interval)
    if hist.empty:
